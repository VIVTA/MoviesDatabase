        ...
        if ($_POST['method']=="Delete") {
            if (mysqli_num_rows($result) > 0) {
                ...
                while ($row = mysqli_fetch_assoc($result)) {
                    echo '<tr><td><input type="radio" name="thecustomers[]" value="' . $row['CustomerID'] . '"></td><td>' . $row['CustomerID'] . '</td><td>' . $row['FirstName'] . '</td><td>' . $row['LastName'] . '</td><td>' . $row['Sex'] . '</td><td>' . $row['Email'] . '</td></tr>';
                }
